...
[WARNING]
====
Vert.x 5 uses the JVM `Cleaner` mechanism as a safety net to cleanup certain resources (clients, executors, etc.)
when they become *unreachable* (older versions used Java finalizers).

Because `Cleaner` execution depends on garbage collection, cleanup timing is *non-deterministic* and can be
more eager than finalization was. In asynchronous tests this matters because the test callback may return
while async operations are still in progress.

If a client (e.g. `HttpClient` / `WebClient`) is created as a local variable and no strong reference is kept
until the async callbacks complete, the JVM may reclaim it and Vert.x may close the underlying pool,
leading to intermittent failures (for example `VertxException: Pool closed`), especially in CI environments.

To avoid this, keep clients strongly reachable for the duration of the asynchronous work (e.g. store them in a field),
and prefer explicit `close()` when appropriate.
====
...
The {@link io.vertx.ext.unit.TestContext#asyncAssertSuccess()} method returns a `Handler<AsyncResult<T>>`
...
The {@link io.vertx.ext.unit.TestContext#asyncAssertSuccess(io.vertx.core.Handler)} method returns a `Handler<AsyncResult<T>>`
instance that acts like {@link io.vertx.ext.unit.Async}, invoking the delegating `Handler<T>` on success
...
The {@link io.vertx.ext.unit.TestContext#asyncAssertFailure()} method returns a `Handler<AsyncResult<T>>`
...
The {@link io.vertx.ext.unit.TestContext#asyncAssertFailure(io.vertx.core.Handler)} method returns a `Handler<AsyncResult<T>>`
instance that acts like {@link io.vertx.ext.unit.Async}, invoking the delegating `Handler<Throwable>` on
...
NOTE: Several test suites can be executed in the same verticle, Vert.x Unit waits until completion of all suite executed.
