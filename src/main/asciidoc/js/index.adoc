...
----
compile io.vertx:vertx-unit:3.3.0-SNAPSHOT
----
...
It is possible to wait until the completion of a specific `link:../../jsdoc/async-Async.html[Async]`, similar
...
zero using `link:../../jsdoc/async-Async.html#countDown[countDown]`:
...
`link:../../jsdoc/test_context-TestContext.html[TestContext]` provides useful methods that provides powerful constructs for async testing:
...
The test suite can also be run with a specified `link:../../jsdoc/vertx-Vertx.html[Vertx]` instance:
...
Such test suite just need to be executed via the `link:../../jsdoc/test_suite-TestSuite.html#run[run]` command, the
...
`link:../../jsdoc/completion-Completion.html[Completion]`:
...
The `link:../../jsdoc/completion-Completion.html[Completion]` object provides also a `link:../../jsdoc/completion-Completion.html#resolve[resolve]` method that
...
The `link:../../jsdoc/completion-Completion.html#awaitSuccess[awaitSuccess]` is a variation that throws an exception when
...
By default, assertions and failures must be done on the `link:../../jsdoc/test_context-TestContext.html[TestContext]` and throwing an
...
The exception handler is set during the _before_ phase, the `link:../../jsdoc/test_context-TestContext.html[TestContext]` is shared
...
For this purpose the `RunTestOnContext` rule needs a `link:../../jsdoc/vertx-Vertx.html[Vertx]`
instance. Such instance can be provided, otherwise the rule will manage an instance under the hood. Such
instance can be retrieved when the test is running, making this rule a way to manage a `link:../../jsdoc/vertx-Vertx.html[Vertx]`
instance as well.
