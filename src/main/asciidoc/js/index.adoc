...
The test suite can also be ran with a specified `link:jsdoc/vertx-Vertx.html[Vertx]` instance:
...
The `link:../cheatsheet/TestOptions.html#setUseEventLoop[setUseEventLoop]` configures the usage of the event
loop:
...
The `link:jsdoc/vertx_unit_runner-VertxUnitRunner.html[VertxUnitRunner]` uses the junit annotations for introspecting the class
...
The runner controls a Vertx instance for the test suite, it can be retrieved using `link:jsdoc/test_context-TestContext.html#vertx[vertx]`.
